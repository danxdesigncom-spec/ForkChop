...
describe('getFlags', () => {
  it('defaults every flag on when nothing is set', () => {
    ...
    expect(flags.simplifiedSidebar).toBe(true);
  });
  ...
  it('falls back to the default on an unexpected value in either direction', () => {
    // Garbage never flips the intent. Every flag currently defaults to true,
    // so we cover the false-default branch by stubbing kroger's default to
    // false first via an explicit env var, then supplying garbage.
